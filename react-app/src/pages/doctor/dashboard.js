        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {selectedMsg && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
                    <div className="bg-white rounded-[2.5rem] shadow-2xl p-10 w-full max-w-lg relative">
                        <button onClick={() => setSelectedMsg(null)} className="absolute top-6 right-6 p-2 rounded-xl hover:bg-slate-100">
                            <Iconify icon="eva:close-fill" className="w-6 h-6 text-slate-400" />
                        </button>
                        <div className="flex items-center gap-3 mb-6">
                            <div className="w-12 h-12 rounded-2xl bg-sky-50 flex items-center justify-center text-sky-500 font-black">{selectedMsg.senderName?.charAt(0)}</div>
                            <div>
                                <p className="font-black text-slate-900">{selectedMsg.senderName}</p>
                                <p className="text-xs text-slate-400">{dayjs(selectedMsg.createdAt).format('MMM DD, YYYY · hh:mm A')}</p>
                            </div>
                        </div>
            ...
            <div className="rounded-[2.5rem] bg-white border border-slate-50 shadow-xl shadow-slate-100/50 overflow-hidden">
                <div className="p-8 border-b border-slate-50 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-sky-500 text-white flex items-center justify-center relative">
                            <Iconify icon="eva:inbox-fill" className="w-6 h-6" />
                            {unreadCount > 0 && (
                                <span className="absolute -top-2 -right-2 w-5 h-5 rounded-full bg-rose-500 text-white text-[9px] font-black flex items-center justify-center border-2 border-white">{unreadCount}</span>
                            )}
                        </div>
                        <div>
                            <h2 className="text-xl font-black text-slate-900">Patient Messages</h2>
                            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-0.5">{unreadCount} unread</p>
                        </div>
                    </div>
                </div>
                {messages.length === 0 ? (
                    <div className="py-20 text-center flex flex-col items-center opacity-40">
                        <Iconify icon="eva:inbox-outline" className="w-16 h-16 mb-4 text-slate-200" />
                        <p className="font-black uppercase tracking-widest text-sm text-slate-300">No messages yet</p>
                    </div>
                ) : (
                    <div className="divide-y divide-slate-50">
                        {messages.map(msg => (
                            <div
                                ...
                                className={`flex items-center gap-6 px-8 py-6 cursor-pointer hover:bg-sky-50/40 transition-all ${!msg.read ? 'bg-sky-50/20' : ''}`}
                            >
                                <div className="w-12 h-12 rounded-2xl bg-slate-100 flex items-center justify-center text-slate-500 font-black">{msg.senderName?.charAt(0)}</div>
                                <div className="flex-1 overflow-hidden">
                                    <div className="flex items-center gap-2">
                                        <p className={`font-bold text-slate-900 ${!msg.read ? 'font-black' : ''}`}>{msg.senderName}</p>
                                        {!msg.read && <span className="w-2 h-2 rounded-full bg-sky-500" />}
                                    </div>
                                    <p className="text-xs text-sky-500 font-bold">{msg.subject}</p>
                                    <p className="text-xs text-slate-400 truncate mt-0.5">{msg.body}</p>
                                </div>
                                <p className="text-[10px] text-slate-400 font-bold whitespace-nowrap">{dayjs(msg.createdAt).format('MMM DD')}</p>
                            </div>
                        ))}
                    </div>
                )}
