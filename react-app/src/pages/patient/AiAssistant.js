...
const AiAssistant = () => {
  ...
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      content: `👋 Hello ${currentUser?.username || ''}! I'm **HealthAI**, your Smart Healthcare Assistant.\n\nI can assist you with:\n- 🤒 Symptom Checker & Health Triage Risk Assessment\n- 🛡️ Disease Information & Prevention Guidelines\n- 💊 Medical Prescription Jargon Simplifier\n- 📄 1-Click PDF Medical Report Export\n\n> ⚠️ **Important:** I am NOT a substitute for professional medical diagnosis. Always consult a doctor for urgent medical advice.`
    }
  ]);
  ...
  const calculateTriageRisk = () => {
    ...
    let advice = 'Mild symptoms detected. Rest, stay hydrated, and monitor your condition.';
    if (score >= 4) {
      riskLevel = 'EMERGENCY / HIGH RISK 🔴';
      advice = 'Urgent emergency evaluation recommended! Please visit nearest emergency department or call 112 / 108.';
    } else if (score >= 2) {
      riskLevel = 'MODERATE RISK 🟡';
      advice = 'Consultation with a general physician recommended within 24 hours.';
    }

    const triagePrompt = `[AI TRIAGE REPORT]\nPatient reported symptoms: ${selected.join(', ')}.\nCalculated Risk Level: ${riskLevel}.\nSystem Recommendation: ${advice}.\n\nPlease provide detailed medical guidance for these symptoms.`;
  ...
  const handleJargonSubmit = (e) => {
    ...
    const prompt = `Please simplify and explain this medical prescription / doctor note in simple plain English & Hindi:\n"${jargonInput}"`;
  ...
  const clearChat = () => {
    setMessages([{
      role: 'assistant',
      content: `👋 Chat reset! I'm **HealthAI**, ready to answer your medical questions.\n\n> ⚠️ I am NOT a replacement for professional medical advice.`
    }]);
  };
  ...
  const downloadReport = () => {
    ...
    doc.text('NearestDoctor - AI Health Report', margin, 13);
    doc.setFontSize(9); doc.setFont('helvetica', 'normal');
    doc.text(`Patient: ${currentUser?.username || 'Patient'}   |   Generated: ${new Date().toLocaleString()}`, margin, 23);
    y = 42;
    ...
    chatMessages.forEach((msg, idx) => {
      ...
      doc.text(`${idx + 1}. ${isUser ? 'Patient' : 'HealthAI'}:`, margin, y);
      y += 5;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-[2.5rem] p-8 w-full max-w-md shadow-2xl border border-slate-100 relative">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-xl font-black text-slate-900">AI Symptom Triage</h3>
                <p className="text-xs text-rose-500 font-bold uppercase tracking-widest">Select your active symptoms</p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-[2.5rem] p-8 w-full max-w-md shadow-2xl border border-slate-100 relative">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-xl font-black text-slate-900">Prescription Simplifier</h3>
                <p className="text-xs text-emerald-500 font-bold uppercase tracking-widest">Decode Medical Jargon</p>
              </div>
            </div>
            ...
            <form onSubmit={handleJargonSubmit} className="space-y-4">
              <textarea
                rows={4}
                required
                placeholder="Paste prescription terms or doctor notes here... (e.g. Tab Amoxicillin 500mg BD pc x 5 days)"
                value={jargonInput}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-black text-slate-900">HealthAI Assistant</h1>
            ...
            <p className="text-slate-400 text-xs font-medium">Smart Medical AI & Instant Symptom Guidance</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            ...
            <span>Prescription Decoder</span>
          </button>
          {hasChat && (
            <button
      ...
      <div className="flex gap-3 items-end">
        <div className="flex-1 bg-white border border-slate-200 rounded-2xl shadow-sm focus-within:ring-2 focus-within:ring-sky-400 focus-within:border-sky-400 transition-all flex items-center px-4">
          <textarea
            ...
            disabled={loading}
            rows={1}
            placeholder="Describe symptoms, medical query or speak... (Enter to send)"
