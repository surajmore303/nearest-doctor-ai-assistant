    ...
    <>
      ...
      <div className="h-full bg-slate-900/95 backdrop-blur-xl border-r border-white/10 shadow-2xl flex flex-col overflow-hidden">
        ...
        <nav className="flex-1 px-4 py-4 space-y-2 overflow-y-auto custom-scrollbar">
          {[
            { to: '/patient/app', label: 'Overview', icon: 'eva:grid-fill' },
            { to: '/patient/ai-assistant', label: 'AI Health Assistant', icon: 'mdi:robot-happy' },
            { to: '/patient/doctors', label: 'Find Doctors', icon: 'eva:search-fill' },
            { to: '/patient/appointments', label: 'Schedule', icon: 'eva:calendar-fill' },
            { to: '/patient/test', label: 'Depression Test', icon: 'eva:clipboard-fill' },
            { to: '/patient/records', label: 'Medical Records', icon: 'eva:folder-fill' },
            { to: '/patient/profile', label: 'My Profile', icon: 'eva:person-fill' },
            { to: '/patient/settings', label: 'Settings', icon: 'eva:settings-2-fill' },
          ].map((item) => (
            <Link
              key={item.to}
              to={item.to}
            ...
            >
              ...
              />
              {isOpen && <span className="font-bold text-sm tracking-wide whitespace-nowrap overflow-hidden">{item.label}</span>}
            </Link>
          ))}
        </nav>
          ...
          <div className="p-6 mt-auto animate-in fade-in duration-500">
            <div className="bg-white/5 rounded-[2rem] p-5 border border-white/5">
              ...
              <div className="h-1.5 w-full bg-white/10 rounded-full overflow-hidden">
                  <div className="h-full bg-sky-500 w-2/3 rounded-full"></div>
              </div>
              <p className="text-[10px] text-slate-500 font-bold mt-3 leading-tight uppercase tracking-wider text-center">Data Encryption Active</p>
            </div>
          </div>
        )}
      </div>
